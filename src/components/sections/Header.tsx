...
function MobileLink({ to, label, trailing }: { to: string; label: string; trailing?: React.ReactNode }) {
  return (
    <Link
      to={to}
      className='flex items-center justify-between rounded-md px-3 py-3 text-sm hover:bg-muted'
    >
...
// Small utility to get products for a given category id
const categoryItems = (categoryId: string) =>
  products.filter((p) => p.categoryId === categoryId)

// ===== New: Mobile Category Panel (slides in) =====
function CategoryPanel({
  ...
  return (
    ...
    >
      {/* Header */}
      <div className='sticky top-0 z-10 flex items-center gap-2 border-b bg-background px-3 py-2.5'>
        <button
          type='button'
          onClick={onBack}
          className='inline-flex items-center rounded-md px-2 py-1 text-sm hover:bg-muted'
          aria-label='Back to categories'
        >
          <ChevronLeft className='h-5 w-5' />
        </button>
        <div className='min-w-0 flex-1 truncate font-semibold'>{category?.name}</div>
        <button
          type='button'
          onClick={onClose}
      ...
      <div className='space-y-1 p-2'>
        {items.map((product) => (
          <MobileLink
            ...
            label={product.name}
            trailing={<ArrowRight className='h-4 w-4 shrink-0' />}
          />
        ))}
        ...
        {category && (
          <Link
            ...
            className='mt-2 block rounded-lg border bg-muted/60 px-3 py-3 text-center text-sm font-medium text-primary hover:bg-muted'
            onClick={onClose}
          >
...
// ===== Main Component =====
const Header = () => {
  // controls which category panel is visible in the mobile drawer
  const [activeCategory, setActiveCategory] = React.useState<string | null>(null)

  // accessibility nicety: close the sub-panel with Escape
  React.useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && activeCategory) setActiveCategory(null)
    }
        ...
        <div className='mx-auto flex max-w-7xl flex-col gap-2 px-4 py-1.5 text-xs md:flex-row md:items-center md:justify-between md:px-8'>
          <div className='flex flex-wrap items-center gap-4'>
            <span className='flex items-center gap-1.5'>
              <MapPin className='h-3.5 w-3.5' /> 1350 University Ave, Berkeley, CA 94702
            </span>
          ...
          <Sheet>
            ...
            <SheetContent side='bottom' className='h-auto p-4'>
              <div className='space-y-3 text-sm'>
                ...
                <a href='tel:+15108487446' className='flex items-start gap-2 rounded-lg bg-muted p-2'>
                  <Phone className='mt-0.5 h-4 w-4 text-muted-foreground' />
                  <span>(510) 848-7446</span>
                </a>
                <a href='mailto:info@signcousa.com' className='flex items-start gap-2 rounded-lg bg-muted p-2'>
          ...
          <div className='flex items-center justify-between py-2 lg:py-3'>
            <div className='flex items-center gap-3 lg:gap-6 '>
              <Sheet>
                ...
                {/* ===== UPDATED: Mobile Nav Drawer ===== */}
                <SheetContent side='left' className='w-[320px] p-0'>
                  {/* Drawer header */}
                  <div className='border-b px-4 py-3'>
                    <Link to='/' className='flex items-center gap-2'>
                      <img src='/images/logo.webp' alt='SignCo USA Logo' className='h-10 w-auto' />
                    </Link>
                  </div>

                  {/* Drawer body */}
                  <div className='relative h-[calc(100vh-56px)] overflow-hidden'>
                    {/* Layer 1: Category list + utility links */}
                    <div className='absolute inset-0 overflow-y-auto'>
                      <div className='space-y-1 p-2'>
                        {/* <MobileLink to='/products' label='Products & Services' />
                        <MobileLink to='/contact' label='Contact' /> */}
                        {/* Category section */}
                        <div className='mt-2 rounded-xl border bg-muted/30'>
                          <div className='px-3 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground'>
                            Categories
                          </div>
                          <nav aria-label='Product categories' className='pb-1'>
                            {categories.map((cat) => (
                              <button
                                key={cat.id}
                                onClick={() => setActiveCategory(cat.id)}
                                className='flex w-full items-center justify-between gap-2 rounded-md px-3 py-3 text-left text-sm hover:bg-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-ring'
                                aria-haspopup='true'
                    ...
                    {/* Layer 2: Sliding product panel for selected category */}
                    <AnimatePresence>
                      {activeCategory && (
                        <CategoryPanel
                          key={activeCategory}
                          ...
                          // Close the whole drawer by simulating a click on the overlay close button via history back
                          onClose={() => {
                            // let Sheet handle closing via the built-in close button using a navigation
                            const closeBtn = document.querySelector<HTMLButtonElement>(
                              '[data-state="open"] [data-dismiss]' // shadcn places a close button internally; if not available, fall back
                            )
                            closeBtn?.click()
                          }}
                        />
              ...
              <Link to='/' className='flex items-center gap-2' aria-label='SignCo USA home'>
                <img src='/images/logo.webp' alt='SignCo USA Logo' className='h-10 w-auto lg:h-20' />
              </Link>
            </div>
